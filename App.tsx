...
const App: React.FC = () => {
  ...
  const handlePipelineGenerate = async () => {
    ...
    // --- PHASE A: OUTLINE ---
    setPipelineState(prev => ({ ...prev, status: 'outlining', currentError: undefined }));
    
    try {
      // 1. Outline
      const outline = await geminiService.generateOutline(inputText);
      ...
      for (let i = 0; i < chunks.length; i++) {
        const chunkData = await geminiService.processChunk(chunks[i].text, i, chunks.length, settings);
