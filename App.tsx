...
const App: React.FC = () => {
  ...
  const handlePipelineGenerate = async () => {
    ...
    try {
      const outlineSections = await geminiService.generateOutline(inputText, inputTitle, parsedTags, settings.selectedModel);
  ...
  const saveSessionData = async (markdown: string, pState: PipelineState, structuredOutput?: StructuredOutput) => {
    ...
    const sessionData: Session = {
      ...
      tags: parsedTags,
      inputText,
      modelUsed: settings.selectedModel, // Persist selected model
