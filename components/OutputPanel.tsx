...
import { Copy, Download, Layers, RefreshCw, ChevronDown, BookOpen, Eye, FileText, Brain, List, Stethoscope, CheckCircle2, XCircle, AlertTriangle } from 'lucide-react';
...
export const OutputPanel: React.FC<OutputPanelProps> = ({ 
  ...
  const coverageAvailable = !!pipelineState?.outline?.length;
  const coverageEnabled = coverageAvailable;

  useEffect(() => {
    if (activeTab === 'coverage' && !coverageEnabled) {
      setActiveTab('study');
    }
  }, [activeTab, coverageEnabled]);
        ...
        <div className="h-full flex flex-col relative bg-slate-50">
            <div className="sticky top-0 bg-white/95 backdrop-blur border-b border-slate-200 z-10 p-3">
                 ...
                 <button 
                   ...
                   className="px-3 py-1.5 text-sm font-medium rounded-md transition-all text-slate-500"
                 >
                   Study Guide
                 </button>
                 <button 
                   onClick={() => setActiveTab('flashcards')}
                   className="px-3 py-1.5 text-sm font-medium rounded-md transition-all bg-white shadow text-teal-700"
                 >
                 ...
                 {coverageAvailable && (
                    <button
                      ...
                      className={`px-3 py-1.5 text-sm font-medium rounded-md transition-all text-slate-500 ${!coverageEnabled ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                      Coverage
                    </button>
                 )}
              ...
              <div className="flex bg-slate-100 rounded-lg p-1">
                 ...
                 {coverageAvailable && (
                    <button
                      onClick={() => setActiveTab('coverage')}
                      disabled={!coverageEnabled}
                      className={`px-3 py-1.5 text-sm font-medium rounded-md transition-all ${activeTab === 'coverage' ? 'bg-white shadow text-teal-700' : 'text-slate-500'} ${!coverageEnabled ? 'opacity-50 cursor-not-allowed' : ''}`}
                    >
                      Coverage
                    </button>
                 )}
