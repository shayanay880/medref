...
import { Copy, Download, Layers, RefreshCw, Eye, FileText, CheckCircle2, AlertTriangle, Wand2, BookMarked, Pin, Sparkles } from 'lucide-react';
import { StructuredOutput, PipelineState, HighlightDensity, OutlinePriority, GlossaryEntry } from '../types';
...
const applyHighlightMarkers = (content: string, density: HighlightDensity) => {
  ...
  return content.replace(/\[\[(R|Y|B|EXTRA)\]\]([\s\S]*?)\[\[\/\1\]\]/g, (_match, marker: HighlightMarker, inner: string) => {
    // Extras are always allowed and don't count towards limits
    if (marker === 'EXTRA') {
      return `<span class="${markerClassMap[marker]}">${inner}</span>`;
    }
...
export const OutputPanel: React.FC<OutputPanelProps> = ({ 
  ...
  const masteryStatus = pipelineState?.masteryStatus;
  ...
  const [quickReviewPins, setQuickReviewPins] = useState<string[]>(json?.pinnedGlossaryTerms || []);
  const [quizIndex, setQuizIndex] = useState(0);
  const [quizMode, setQuizMode] = useState<'term' | 'definition'>('term');
  ...
  const glossaryStorageKey = useMemo(() => `medref-glossary-${pipelineState?.rawInputHash?.hash || 'default'}`, [pipelineState?.rawInputHash?.hash]);
  const glossaryEntries = useMemo<GlossaryEntry[]>(() => (json?.glossaryEntries || []).filter((entry): entry is GlossaryEntry => Boolean(entry && entry.term && entry.definition)), [json?.glossaryEntries]);
  const pinnedSet = useMemo(() => new Set(quickReviewPins), [quickReviewPins]);
  const pinnedEntries = useMemo(() => glossaryEntries.filter((entry) => pinnedSet.has(entry.term)), [glossaryEntries, pinnedSet]);
  const quizPool = pinnedEntries.length ? pinnedEntries : glossaryEntries;
  const activeQuiz = quizPool.length ? quizPool[quizIndex % quizPool.length] : null;

  useEffect(() => {
    try {
      const stored = localStorage.getItem(glossaryStorageKey);
      if (stored) {
        setQuickReviewPins(JSON.parse(stored));
      }
    } catch (err) {
      console.warn('Glossary pins restore failed', err);
    }
  }, [glossaryStorageKey]);

  useEffect(() => {
    if (json?.pinnedGlossaryTerms?.length) {
      setQuickReviewPins(json.pinnedGlossaryTerms);
    }
  }, [json?.pinnedGlossaryTerms]);

  useEffect(() => {
    try {
      localStorage.setItem(glossaryStorageKey, JSON.stringify(quickReviewPins));
    } catch (err) {
      console.warn('Glossary pins persist failed', err);
    }
  }, [glossaryStorageKey, quickReviewPins]);
  ...
  const toggleGlossaryPin = (term: string) => {
    setQuickReviewPins((prev) => {
      const exists = prev.includes(term);
      const updated = exists ? prev.filter((t) => t !== term) : [...prev, term];
  ...
  const handleExportGlossary = () => {
    ...
    const rows = glossaryEntries.map((entry) => `${entry.term}${entry.englishTerm ? ` (${entry.englishTerm})` : ''}: ${entry.definition}`);
    const blob = new Blob([rows.join('\n')], { type: 'text/plain;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'medref-glossary.txt';
    a.click();
  };
    ...
    const htmlContent = `
      ...
      <head>
        <style>
          ...
          .extra-inline { color: #b91c1c; font-style: italic; font-weight: 600; }
        </style>
      </head>
  ...
  const sanitizeForExport = (content: string) => {
    ...
    const allowedClasses = new Set(['hl-red', 'hl-yellow', 'hl-blue', 'chunk-separator', 'extra-inline']);
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-4">
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-xs uppercase tracking-wide text-emerald-700 font-bold">Glossary quick review</p>
                      <p className="text-sm text-slate-600">Pin key terms and drill them like flashcards next to active recall.</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={handleExportGlossary}
                      className="inline-flex items-center gap-1 text-xs font-medium px-3 py-1.5 rounded-md border border-slate-200 text-slate-700 hover:bg-slate-50"
                    >
                      <Download size={14} />
                      Export list
                    </button>
                    <span className="text-[11px] text-slate-500">{glossaryEntries.length} terms</span>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-80 overflow-y-auto pr-1">
                  {glossaryEntries.map((entry) => {
                    const pinned = pinnedSet.has(entry.term);
                    return (
                      <div key={entry.term} className="border border-slate-200 rounded-lg p-3 flex items-start justify-between gap-3">
                        <div className="text-right flex-1">
                          <p className="font-semibold text-slate-800">{entry.term}{entry.englishTerm ? ` (${entry.englishTerm})` : ''}</p>
                          <p className="text-sm text-slate-600 leading-6">{entry.definition}</p>
                        </div>
                        <button
                          onClick={() => toggleGlossaryPin(entry.term)}
                          className={`p-2 rounded-md border ${pinned ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'text-slate-500 border-slate-200 hover:bg-slate-50'}`}
                          title={pinned ? 'Pinned for quick review' : 'Pin to quick review'}
                        >
                          <Pin size={14} />
                        </button>
                      </div>
                ...
                <div className="border border-emerald-100 bg-emerald-50 rounded-lg p-4 flex flex-col gap-3">
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    <div className="flex items-center gap-2 text-emerald-800 font-semibold">
                      <Sparkles size={16} />
                      <span>One-click mini-quiz</span>
                    </div>
                    <div className="flex items-center gap-2 text-xs">
                      <button
                        onClick={() => { setQuizMode('term'); setQuizRevealed(false); }}
                        className={`px-3 py-1 rounded-full border ${quizMode === 'term' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-emerald-700 border-emerald-200'}`}
                      >
                        Term → Definition
                      </button>
                      <button
                        onClick={() => { setQuizMode('definition'); setQuizRevealed(false); }}
                        className={`px-3 py-1 rounded-full border ${quizMode === 'definition' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-emerald-700 border-emerald-200'}`}
                      >
                        Definition → Term
                      </button>
                    </div>
                  ...
                  {activeQuiz ? (
                    <div className="bg-white rounded-lg border border-emerald-100 p-4 shadow-sm flex flex-col gap-2">
                      <p className="text-xs text-emerald-700 font-semibold">
                        {quizPool === pinnedEntries && pinnedEntries.length ? 'Pinned terms' : 'All glossary terms'} · Card {((quizIndex % quizPool.length) + 1)} / {quizPool.length}
                      </p>
                      <p className="text-lg font-bold text-slate-900">
                        {quizMode === 'term' ? `${activeQuiz.term}${activeQuiz.englishTerm ? ` (${activeQuiz.englishTerm})` : ''}` : activeQuiz.definition}
                      </p>
                      {quizRevealed && (
                        <p className="text-base text-slate-700 leading-7">
                          {quizMode === 'term' ? activeQuiz.definition : `${activeQuiz.term}${activeQuiz.englishTerm ? ` (${activeQuiz.englishTerm})` : ''}`}
                        </p>
                      )}
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => setQuizRevealed((prev) => !prev)}
                          className="px-3 py-1.5 text-sm rounded-md bg-emerald-600 text-white font-medium"
                        >
                          {quizRevealed ? 'Hide answer' : 'Show answer'}
                        </button>
                        <button
                          onClick={handleAdvanceQuiz}
                          className="px-3 py-1.5 text-sm rounded-md border border-emerald-200 text-emerald-700 hover:bg-emerald-100"
                        >
                          Next card
                        </button>
                      </div>
                    ...
                    <p className="text-sm text-slate-600">Add at least one glossary entry to start quizzing.</p>
                  )}
                </div>
              </div>
            )}
            ...
            {sortedOutline.length ? (
              <ul className="space-y-3">
                {sortedOutline.map(section => {
                  ...
                  const mastery = masteryStatus?.sectionProgress?.[section.id];
                  const completionPct = Math.round((mastery?.completion || 0) * 100);
                  const masteryPct = Math.round((mastery?.masteryScore || 0) * 100);
                  return (
                    <li key={section.id} className="flex items-start gap-3 p-3 bg-slate-50 rounded-lg">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-[11px] text-slate-500 mb-2">
                          Completion {completionPct}% · Mastery {masteryPct}%
                        </div>
